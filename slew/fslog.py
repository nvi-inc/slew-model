...
is_source = re.compile(r':source=(?P<name>.*),(?P<ra>.*),(?P<dec>.*),(?P<epoch>.*),(?P<wrap>neutral|cw|ccw)').match  # test source
is_acquired = re.compile(
    '(?:flagr/antenna,acquired|#flagr#flagr/antenna,acquired|#trakl#Source acquired|#trakl# Source acquired|#start#Source reached)'
).match
...
is_location = re.compile(r'.location,(?P<name>.*),(?P<lon>.*),(?P<lat>.*),(?P<alt>.*)').match

def read_log(dbase, path, verbose=False):

    def make_record():
        if all((name, start_slewing, stop_slewing)):
            scan = dbase.get_or_create(Scan, name=name, station=station, session=session)
            scan.source, scan.src_ra = source['name'], source['ra']
            scan.src_dec, scan.src_epoch = source['dec'], source['epoch']
    ...
    with open(path) as fh:
        ...
        for line in fh:
            ...
            if is_postob(data):
                ...
                if not source:
                    source, start_slewing = src_rec, utc(pcfs=record['time'])
